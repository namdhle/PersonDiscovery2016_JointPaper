...
\mypartitle{Verification-based naming (VBN).} To overcome the weakness from clustering-based naming, verification-based naming puts higher priority on detected names. Face / speech tracks corresponding to names are used as enrolment data to train discriminative models. Names are then propagated to all face / speech tracks which are verified to have the same identity (Fig.\ref{fig:vbn}).
